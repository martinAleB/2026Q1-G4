...
Compone el resto: declara la VPC mediante el módulo `network`, las 5 tablas DynamoDB, la cola SQS y su DLQ, las 14 Lambdas, el HTTP API de API Gateway, el User Pool de Cognito con su client secret en Secrets Manager, los CloudWatch Log Groups con retention, el EventBridge cron del portfolio updater y el bucket S3 del frontend. Centraliza en `locals.tf` los catálogos de Lambdas, integraciones y rutas para evitar duplicación de código. Ver [`terraform/README.md`](terraform/README.md) para el detalle archivo por archivo.
...
|---|---|
| `for_each` | Instancia múltiples recursos del mismo tipo desde un mapa. Se usa para las 14 Lambdas, sus permisos, integraciones y rutas del HTTP API, los 15 CloudWatch Log Groups, las subnets, route tables, security groups y VPC endpoints del módulo `network`, y las 5 tablas DynamoDB invocadas como módulos |
...
- **`aws_sqs_queue.main_dlq`**: dead letter queue compartida. El SQS principal y la Lambda `portfolio-updater` apuntan ahí. El EventBridge target del cron mensual también la usa.
- **`aws_cloudwatch_log_group.lambdas`**: 15 log groups creados explícitamente para fijar `retention_in_days` (default 7) y evitar logs "Never expire" que sangran el budget.
...
- **DynamoDB**: 5 tablas — `simulations`, `fintech`, `product`, `user`, `portfolio` (con GSI `gsi1`).
...
- **CloudWatch**: 15 log groups con `retention_in_days = 7`, 1 EventBridge rule (cron mensual) + target.
